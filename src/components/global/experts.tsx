                    ...
                    {data.map(item => (
                        <div key={item.id}>
                            <div className="relative h-[300px] mb-7">
                                <Image src={`${API_URL}${item.photo.url}`} alt="photo" fill className=" object-cover object-center" />
                            </div>
                            <div className={cn("text-sm font-bold font-mono mb-2.5", !isBackground && "text-aqua-base")}>
                                {item.name.split(" ")[0]} <br />
                                {item.name.split(" ").slice(1).join(" ")}
                            </div>
                            <div className={cn("text-xs max-w-5/6", isBackground && "text-[#A6B0DF]")}>
                                {item.description}
                            </div>
                        </div>
                    ))}
