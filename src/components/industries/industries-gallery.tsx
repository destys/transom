  ...
  return (
    <div>
      <Tabs defaultValue={data[0].id.toString()}>
        <TabsList className="flex xl:grid xl:grid-cols-6 mb-12 xl:mb-36 w-full max-w-[1920px] mx-auto h-auto max-xl:overflow-x-auto">
          {data.map((item) => (
            <TabsTrigger
              value={item.id.toString()}
              key={item.id}
              className="max-xl:shrink-0 max-xl:basis-2/5 max-sm:basis-2/3 block border-0 rounded-none bg-transparent cursor-pointer relative group h-[400px] xl:h-[600px] overflow-hidden whitespace-normal p-0 shadow-none data-[state=active]:[&>div]:opacity-100 data-[state=active]:[&>div>img]:saturate-100 data-[state=active]:[&>div]:translate-y-0 data-[state=active]:[&>div]:text-sand-base"
            >
