  ...
  return (
    <div className="mb-56">
      <div className="container">
        {data.map((item) => (
          <div key={item.id} className="mb-20 lg:mb-40">
            <div className="flex items-center gap-8 mb-6">
              <div className="text-lg lg:text-2xl font-bold text-aqua-base">
                {item.title}
              </div>
              <div className="flex-auto bg-[#D9D9D9] h-px"></div>
            </div>
            <div className="text-base lg:text-[22px] mb-10">
              {item.description}
            </div>
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-9">
                {item.clients.map((client) => (
                  <div
                    key={client.id}
                    className="text-lg text-sand-base font-bold"
                  >
                    {client.title}
                  </div>
                ))}
