...
export const ServiceComparison = ({ data }: Props) => {
  return (
    <div className="relative mb-32">
      ...
      <div className="container grid grid-cols-2 relative z-20 py-12 lg:py-24">
        <div className="space-y-[30px]">
          <h2 className="text-[32px] text-center">{data.before.title}</h2>
          {data.before.rows.map((row) => (
            <div
              key={row.id}
              className="flex flex-col items-center text-center text-[#FF0000] gap-[30px]"
            >
              <XIcon />
        ...
        <div className="space-y-[30px]">
          <h2 className="text-[32px] text-center text-sand-base">
            {data.after.title}
          </h2>
          {data.after.rows.map((row) => (
            <div
              key={row.id}
              className="flex flex-col items-center text-center text-[#0DFF00] gap-[30px]"
            >
              <CheckIcon />
