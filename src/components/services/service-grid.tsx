    ...
    <div className="mb-14 lg:mb-28">
      <div className="container">
        <div className="grid lg:grid-cols-12 gap-5 mb-16">
          <h2 className="font-sans lg:col-span-5 text-3xl font-semibold">
            {data.title}
          </h2>
          <div className="lg:col-span-7">{data.description}</div>
        </div>
        <div
          className={cn(
            "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5",
            data.rows.length === 3 && "lg:grid-cols-3"
          )}
        >
          {data.rows.map((row) => (
            <div key={row.id} className="flex flex-col">
              <div className="relative h-[220px]">
                <Image
                  ...
                  alt={row.image.alternateText || "Изображение"}
                  fill
                />
              </div>
              <div className="p-5 border-2 border-[#E8EEFF] flex-auto">
                ...
                {row.description && (
                  <div
                    className={cn(
                      "text-lg font-medium leading-[1.2]",
                      row.title && "text-sm font-normal"
                    )}
                  >
                    {row.description}
                  </div>
