...
import { VacancyProps } from "@/types/vacancies-page.types";
...
export const SearchForm: React.FC<Props> = ({ data, maxItems = 8 }) => {
  ...
  const items = useMemo(() => {
    ...
    const scored = data
      .map((v) => {
        const hn = norm(v.title);
        if (!hn.includes(qn)) return null;

        // простой скоринг только по title
        const titleIdx = hn.indexOf(qn);
        const score = titleIdx !== -1 ? titleIdx : 10_000;
        return { v, score };
      })
  ...
  const onKeyDown: React.KeyboardEventHandler<HTMLInputElement> = (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      listRef.current?.children[(active + 1) % items.length]?.scrollIntoView({
        block: "nearest",
      });
